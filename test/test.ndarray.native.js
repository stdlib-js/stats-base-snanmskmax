...
var snanmskmax = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 7', opts, function test( t ) {
	t.strictEqual( snanmskmax.length, 7, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the maximum value of a strided array according to a mask, ignoring NaN values', opts, function test( t ) {
	var mask;
	var x;
	var v;
	...
	v = snanmskmax( x.length, x, 1, 0, mask, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = snanmskmax( 0, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );

	v = snanmskmax( -1, x, 1, 0, mask, 1 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );
...
tape( 'the function supports negative `stride` parameters', opts, function test( t ) {
	...
	N = floor( x.length / 2 );
	v = snanmskmax( N, x, -2, 10, mask, -2, 10 );
