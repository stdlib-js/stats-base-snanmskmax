...
var snanmskmax = tryRequire( resolve( __dirname, './../lib/snanmskmax.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 5', opts, function test( t ) {
	t.strictEqual( snanmskmax.length, 5, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the maximum value of a strided array according to a mask, ignoring NaN values', opts, function test( t ) {
	...
	v = snanmskmax( x.length, x, 1, mask, 1 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = snanmskmax( 0, x, 1, mask, 1 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );

	v = snanmskmax( -1, x, 1, mask, 1 );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );
